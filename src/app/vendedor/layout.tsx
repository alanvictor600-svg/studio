                ...
                <SidebarHeader>
                    <Link href="/" onClick={() => setOpenMobile(false)} className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                            ...
                            <span className="text-xs text-sidebar-foreground/80 -mt-1">Painel de Vendedor</span>
                        </div>
                    </Link>
                </SidebarHeader>
                <SidebarContent>
                ...
                <header className="sticky top-0 z-10 flex h-16 items-center justify-between border-b bg-secondary px-4 md:px-6">
                    <div className="flex items-center gap-4">
                        <SidebarTrigger className="md:hidden" />
                         <span className="hidden md:inline font-semibold text-primary">Painel do Vendedor</span>
                    </div>
...
export default function VendedorLayout({ children }: { children: React.ReactNode }) {
    ...
    useEffect(() => {
        ...
        if (!isAuthenticated) {
            router.replace('/login?redirect=/vendedor');
            return;
        }

        if (currentUser && currentUser.role !== 'vendedor') {
            const destination = currentUser.role === 'admin' ? '/admin' : `/cliente`;
    ...
    return (
        <SidebarProvider>
            <SellerDashboardProvider user={currentUser}>
                <SellerDashboardUI>{children}</SellerDashboardUI>
